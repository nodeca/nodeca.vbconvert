  ...
  N.wire.on('init:jobs', function register_vbconvert_forum_images_fetch() {
    N.queue.registerWorker({
      ...
      process: function (callback) {
        ...
        //
        function done() {
          N.queue.status('vbconvert_forum_images_fetch', self.task.id, function (err, data) {
            if (err) {
              callback(err);
              return;
            }
        ...
        N.models.forum.Post
          ...
          async.each(posts, function (post, callback) {
            ...
            async.each(Object.keys(post.image_info), function (key, _callback) {
