  ...
  N.wire.on('init:jobs', function register_vbconvert_messages_import() {
    ...
    N.queue.registerTask({
      ...
      async process(ids) {
        ...
        let mappings_by_id = _.keyBy(mappings, mapping => `${mapping.mysql}_${mapping.user}_${mapping.to}`);
        ...
        for (let mapping of mappings) {
          ...
          if (!post) {
            N.logger.error(`Cannot find message (id=${mapping.message}, mysql=${mapping.mysql})`);
            continue;
          }
          ...
          bbcode_data.push({
            id:          `${mapping.mysql}_${mapping.user}_${mapping.to}`,
            text:        mapping.text,
            options:     params,
        ...
        await Promise.map(parsed, async function (result) {
          ...
          await N.models.users.DlgMessage.update({ _id: mappings_by_id[result.id].message }, updateData);
        }, { concurrency: 50 });
