...
module.exports = function to_md(tokens, post_map) {
  var result = '';
  ...
  for (var i = 0; i < tokens.length; i++) {
    ...
    switch (token.type) {
      ...
      case 'quote':
        ...
        if (token.replyto && post_map && post_map[token.replyto[1]]) {
          result += post_map[token.replyto[1]] + '\n';
      ...
      case 'url':
        ...
        var contents = to_md(tokens.slice(i + 1, j));

        if (token.param && token.param !== contents) {
          result += '[' + contents + '](' + token.param + ')';
        } else {
          result += '<' + to_text(tokens.slice(i + 1, j)) + '>';
        }
