...
module.exports = function (N) {
  ...
  function get_attachment_urls(ids, callback) {
    ...
    N.models.vbconvert.FileMapping
      ...
      N.models.users.MediaInfo
          .find({ _id: {
            $in: _.uniq(_.pluck(file_mappings, 'media_id').map(String))
          } })
  ...
  function get_link_mapping(links, callback) {
    var link_rewrite = require('../../link_rewrite')(N);
    var result = {};

    async.each(links, function map(url, callback) {
      link_rewrite(url, function (err, newUrl) {
        if (err) {
          callback(err);
          return;
        }
    ...
    N.queue.registerWorker({
      ...
      process: function (callback) {
        ...
        N.models.vbconvert.PostMapping
          ...
          var posts = [];
          var resources = { posts: [], topics: [], attachments: [], links: [] };
          ...
          async.parallel([
            ...
            get_link_mapping.bind(null, resources.links)
