  ...
  N.wire.on('init:jobs', function register_vbconvert_forum_posts_import() {
    N.queue.registerWorker({
      ...
      * map() {
        ...
        N.logger.info(`Start importing forum posts (${chunks.length} chunks)`);
      ...
      * process() {
        ...
        N.logger.info(`Importing forum posts ${this.data.from}-${this.data.to} - started`);
        ...
        N.logger.info(`Importing forum posts ${this.data.from}-${this.data.to} - finished (${
      ...
      reduce(chunksResult) {
        ...
        N.logger.info('Finish importing forum posts');
      }
    });
  });
};
