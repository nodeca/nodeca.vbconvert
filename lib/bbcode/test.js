...
// render relative param-less urls (can't be auto)
assert.equal(to_md(tokenize('[url]/blah[/url]')), '[/blah](/blah)');

// avoid links inside other links
assert.equal(to_md(tokenize('[url="foo"]foo [url="bar"]bar[/url] baz[/url]')), '[foo ](foo)[bar](bar) baz');
...
// render only if link immediately follows
assert.equal(to_md(tokenize('[img] http://blah[/img]')), '\\[img\\] http://blah\\[/img\\]');
