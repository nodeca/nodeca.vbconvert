// Display blog import progress in admin interface
//
'use strict';
...
let view = null;
let SELECTOR = '#vbconvert-task-blogs';
let finished_tasks = {};
...
N.wire.on('navigate.done:admin.vbconvert.import_bbcode', function blogs_task_widget_setup() {
  if (!$(SELECTOR).length) return;

  let current = N.runtime.page_data.blogs_task.current || 0;
  let total   = N.runtime.page_data.blogs_task.total || 1;
  ...
  N.live.on('admin.vbconvert.blogs', update_task_status);
});


N.wire.on('navigate.exit:admin.vbconvert.import_bbcode', function blogs_task_widget_teardown() {
  if (!$(SELECTOR).length) return;

  view = null;
  ko.cleanNode($(SELECTOR)[0]);

  N.live.off('admin.vbconvert.blogs', update_task_status);
});


N.wire.once('navigate.done:admin.vbconvert.import_bbcode', function blogs_task_widget_setup_handlers() {
