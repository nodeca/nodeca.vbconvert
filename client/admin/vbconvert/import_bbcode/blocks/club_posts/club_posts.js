...
let view = null;
let SELECTOR = '#vbconvert-task-club-posts';
let finished_tasks = {};
...
N.wire.on('navigate.done:admin.vbconvert.import_bbcode', function club_post_task_widget_setup() {
  ...
  N.live.on('admin.vbconvert.club_posts', update_task_status);
});


N.wire.on('navigate.exit:admin.vbconvert.import_bbcode', function club_post_task_widget_teardown() {
  if (!$(SELECTOR).length) return;

  view = null;
  ko.cleanNode($(SELECTOR)[0]);

  N.live.off('admin.vbconvert.club_posts', update_task_status);
});


N.wire.once('navigate.done:admin.vbconvert.import_bbcode', function club_post_task_widget_setup_handlers() {
