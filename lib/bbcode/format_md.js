  ...
  for (var i = 0; i < tokens.length; i++) {
    ...
    switch (token.type) {
      ...
      case 'attach':
        ...
        break;
      case 'smiley':
        if (!refs.smileys || !refs.smileys[token.text]) {
          throw new Error('Unknown smiley: ' + token.text);
        }

        result += refs.smileys[token.text];
        break;
      case 'list_item':
  ...
  for (var i = 0; i < tokens.length; i++) {
    ...
    switch (token.type) {
      ...
      case 'code':
        ...
        for (j = i + 1; j < tokens.length; j++) {
          if (tokens[j].type === 'text') { text += tokens[j].text; }
          if (tokens[j] === token.peer) { break; }
        }
