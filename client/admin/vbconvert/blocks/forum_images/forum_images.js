// Display images meta fetch progress in admin interface
//
'use strict';
...
var view = null;
var SELECTOR = '#vbconvert-task-forum-images';
var ignore_runid = 0;
var last_runid = 0;
...
N.wire.on('navigate.done:admin.vbconvert', function vbconvert_forum_images_task_widget_setup() {
  if (!$(SELECTOR).length) return;

  var current = N.runtime.page_data.forum_images_task.current || 0;
  var total   = N.runtime.page_data.forum_images_task.total || 1;
  ...
  N.live.on('admin.vbconvert.forum_images', update_task_status);
});


N.wire.on('navigate.exit:admin.vbconvert', function vbconvert_forum_images_task_widget_teardown() {
  if (!$(SELECTOR).length) return;

  view = null;
  ko.cleanNode($(SELECTOR)[0]);

  N.live.off('admin.vbconvert.forum_images', update_task_status);
});


N.wire.once('navigate.done:admin.vbconvert', function vbconvert_forum_images_task_widget_setup_handlers() {
