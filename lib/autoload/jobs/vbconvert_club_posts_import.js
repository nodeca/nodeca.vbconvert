// Convert bbcode to markdown and rebuild club posts
//
'use strict';
...
module.exports = function (N) {
  ...
  N.wire.on('init:jobs', function register_vbconvert_club_posts_import() {
    // Iterator
    //
    N.queue.registerTask({
      name: 'vbconvert_club_posts_import',
      pool: 'hard',
      baseClass: Queue.IteratorTemplate,

      // static id to make sure it will never be executed twice at the same time
      taskID: () => 'vbconvert_club_posts_import',

      async init() {
        let query = N.models.vbconvert.ClubPostMapping.countDocuments();

        if (this.args.length < 1 || !this.args[0]) {
          // if no min _id
          let min_post = await N.models.vbconvert.ClubPostMapping.findOne()
                                  .select('mysql')
                                  .sort({ mysql: 1 })
                                  .lean(true);
        ...
        if (this.args.length < 2 || !this.args[1]) {
          // if no max _id
          let max_post = await N.models.vbconvert.ClubPostMapping.findOne()
                                  .select('mysql')
                                  .sort({ mysql: -1 })
                                  .lean(true);
      ...
      async iterate(state) {
        ...
        // Fetch posts _id
        //
        let query = N.models.vbconvert.ClubPostMapping.find()
        ...
        let chunks = _.chunk(posts.map(p => String(p.mysql)), CHUNK_SIZE)
                      .map(ids => N.queue.vbconvert_club_posts_import_chunk(ids));
    ...
    //
    N.queue.registerTask({
      name: 'vbconvert_club_posts_import_chunk',
      pool: 'hard',
      removeDelay: 3600,
      async process(ids) {
        let start_time = Date.now();

        N.logger.info(`Importing club posts ${ids[0]}-${ids[ids.length - 1]} - ${ids.length} - started`);

        // fetch source text (bbcode) for posts
        let mappings = await N.models.vbconvert.ClubPostMapping
                                .where('mysql').in(ids)
                                .lean(true);
        ...
        for (let mapping of mappings) {
          ...
          if (!post) {
            N.logger.error(`Cannot find club post (id=${mapping.post_id}, mysql=${mapping.mysql})`);
            continue;
          }
        ...
        N.logger.info(`Importing club posts ${ids[0]}-${ids[ids.length - 1]} - ${ids.length} - finished (${
    ...
    N.queue.on('task:progress:vbconvert_club_posts_import', function (task_info) {
      N.live.debounce('admin.vbconvert.club_posts', {
        uid:     task_info.uid,
    ...
    N.queue.on('task:end:vbconvert_club_posts_import', function (task_info) {
      N.live.emit('admin.vbconvert.club_posts', {
        uid:      task_info.uid,
        finished: true
      });
    });
