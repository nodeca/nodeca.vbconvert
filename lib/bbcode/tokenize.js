...
add_tag(/\[\/?quote\]/, function (text) {
  return { type: 'quote', nesting: text[1] !== '/' ? 1 : -1, text: text };
});
  ...
  for (var i = 0; i < tokens.length; i++) {
    if (tokens[i].nesting === 1) {
      ...
      if (found !== -1) {
        ...
        throw new Error("Tags aren't balanced properly");
      }
    } else {
      out_tokens.push(tokens[i]);
    }
...
// TODO: chop last newline before closing quote, e.g. [quote]foo\n [/quote] -> [quote]foo[/quote]
module.exports = function (text) {
  return add_smilies(balance(remove_unpaired(tokenize(text))));
};
