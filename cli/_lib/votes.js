...
const GROUP_MESSAGE = 5; // content type for club posts / social group messages


module.exports = async function (N) {
  const conn = await N.vbconvert.getConnection();

  let count, bar, last_date;

  //
  // Import forum post votes
  //
  ...
  bar = progress(' votes (forum) :current/:total :percent', count);
  ...
  //
  // Import club post votes
  //
  count = (await conn.query(`
    SELECT count(*) AS total
    FROM votes
    WHERE contenttypeid = ?
  `, [ GROUP_MESSAGE ]))[0][0].total;

  bar = progress(' votes (clubs) :current/:total :percent', count);
  ...
  for (;;) {
    let rows = (await conn.query(`
      ...
      ORDER BY date ASC
      LIMIT ?
    `, [ last_date, GROUP_MESSAGE, BULK_SIZE ]))[0];
    ...
    let pmap = await N.models.vbconvert.ClubPostMapping.find()
                          .where('mysql').in(_.uniq(_.map(rows, 'targetid')))
                          .select('post_id mysql')
                          .lean(true);
