...
module.exports = function (N) {
  ...
  function send_notification_live(taskid, runid) {
    N.queue.status(taskid, function (err, data) {
      if (err) {
        return;
      }
      ...
      if (data && data.state === 'reducing') {
        task_info.finished = true;
      } else {
        task_info.started = true;
      }

      task_info.runid = runid;

      if (task_info.finished) {
        // sent only once when state is 'reducing'
  ...
  function store_external_urls(html, callback) {
    var $html = cheequery(html);
    var urls  = [];

    $html.find('a[href]').addBack('a[href]').each(function () {
    ...
    N.redis.sadd.apply(N.redis, [ 'vbconvert:external_urls' ].concat(urls).concat([ callback ]));
  }
  ...
  N.wire.on('init:jobs', function register_forum_posts_import() {
    N.queue.registerWorker({
      ...
      map: function (callback) {
        var runid = crypto.pseudoRandomBytes(16).toString('hex');
      ...
      process: function (real_callback) {
        var self = this;

        // patch callback to always return self.runid,
        // it's needed in reduce
        ...
        N.models.vbconvert.PostMapping
          ...
          async.parallel([
            ...
            async.eachLimit(posts, 50, function (post_ref, callback) {
              ...
              N.models.forum.Post.findById(post_ref.id).exec(function (err, post) {
                ...
                N.models.core.MessageParams.getParams(post.params_ref, function (err, params) {
                  ...
                  N.parse({
                    ...
                    N.models.forum.Post.update({ _id: post._id }, updateData, function (err) {
                      if (err) {
                        callback(err);
                        return;
                      }

                      store_external_urls(result.html, callback);
                    });
                  });
                });
              });
              ...
              send_notification_live(self.task.id, self.data.runid);
            });
          });
        });
      },
      ...
      reduce: function (chunksResult, callback) {
        send_notification_live(this.id, chunksResult && chunksResult[0] ? chunksResult[0].runid : null);
        callback();
      }
    });
  });
