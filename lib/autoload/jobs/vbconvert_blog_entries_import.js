...
module.exports = function (N) {
  ...
  N.wire.on('init:jobs', function register_vbconvert_blog_entries_import() {
    ...
    N.queue.registerTask({
      ...
      async process(ids) {
        ...
        await Promise.map(parsed, async function (result) {
          ...
          await N.models.blogs.BlogEntry.update({ _id: mappings_by_id[result.id].mongo }, updateData);
        }, { concurrency: 50 });
    ...
    N.queue.on('task:progress:vbconvert_blog_entries_import', function (task_info) {
      N.live.debounce('admin.vbconvert.blog_entries', {
        uid:     task_info.uid,
    ...
    N.queue.on('task:end:vbconvert_blog_entries_import', function (task_info) {
      N.live.emit('admin.vbconvert.blog_entries', {
        uid:      task_info.uid,
        finished: true
      });
    });
