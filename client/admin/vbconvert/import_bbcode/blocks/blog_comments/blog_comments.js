...
let view = null;
let SELECTOR = '#vbconvert-task-blog-comments';
let finished_tasks = {};
...
N.wire.on('navigate.done:admin.vbconvert.import_bbcode', function blog_comments_task_widget_setup() {
  if (!$(SELECTOR).length) return;

  let current = N.runtime.page_data.blog_comments_task.current || 0;
  let total   = N.runtime.page_data.blog_comments_task.total || 1;
  ...
  N.live.on('admin.vbconvert.blog_comments', update_task_status);
});


N.wire.on('navigate.exit:admin.vbconvert.import_bbcode', function blog_comments_task_widget_teardown() {
  if (!$(SELECTOR).length) return;

  view = null;
  ko.cleanNode($(SELECTOR)[0]);

  N.live.off('admin.vbconvert.blog_comments', update_task_status);
});


N.wire.once('navigate.done:admin.vbconvert.import_bbcode', function blog_comments_task_widget_setup_handlers() {
