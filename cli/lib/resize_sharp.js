...
function createPreview(path, resizeConfig, imageType, callback) {
  fs.stat(path, function (err, stats) {
    ...
    if (resizeConfig.skip_size && stats.size < resizeConfig.skip_size) {
      ...
      gmInstance.toFile(tmpFilePath, function (err) {
        if (err) {
    ...
    gmInstance.metadata(function (err, imgSz) {
      if (err) {
        fs.unlink(tmpFilePath, function () {
          callback(err);
        });
      ...
      // Save file
      gmInstance.resize(scaledWidth, scaledHeight).crop('center').toFile(tmpFilePath, function (err) {
        if (err) {
          // Remove temporary file if error
          fs.unlink(tmpFilePath, function () {
            callback(err);
