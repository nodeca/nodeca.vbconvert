...
add_tag(/\[list=(?:"(.*?)"|'(.*?)'|(.*?))\]/, function (text, m1, m2, m3) {
  var param = m1 || m2 || m3 || '';
...
add_tag(/\[\/?list\]/, function (text) {
  return { type: 'list', nesting: text[1] !== '/' ? 1 : -1, text: text };
});

// NOTE: in vB these are NOT tokens, but treating them otherwise would break
//       markdown writer
//
// This broken bbcode would be parsed differently: [list][quote="[*]"][/list]
//
...
add_tag(/\[(font|color)=(?:"(.*?)"|'(.*?)'|(.*?))\]/, function (text, tag, m1, m2, m3) {
  var param = m1 || m2 || m3 || '';
...
add_tag(/\[\/(font|color)\]/, function (text, tag) {
  return { type: tag, nesting: -1, text: text };
});

add_tag(/\[size=(?:"([0-9\+\-]+?)"|'([0-9\+\-]+?)'|([0-9\+\-]+?))\]/, function (text, tag, m1, m2, m3) {
  var param = m1 || m2 || m3 || '';
...
add_tag(/\[\/size\]/, function (text) {
  return { type: 'size', nesting: -1, text: text };
});
