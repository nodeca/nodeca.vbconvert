...
module.exports = function (N) {
  ...
  async function link_to_category(N, urlStr, params) {
    ...
    return { apiPath: 'forum.index', params: { $anchor: hash } };
  }
  ...
  //
  function link_to_album_list(N, urlStr, params) {
    return {
  ...
  //
  async function link_to_blog_user(N, urlStr, params) {
    if (params.nick) {
      let user = await N.models.users.User.findOne()
                           .where('nick_normalized_lc').equals(params.nick.toLowerCase())
                           .select('hid')
                           .lean(true);

      if (user) params = Object.assign({}, params, { user: user.hid });
    }
  ...
  //
  async function link_to_blog_comment(N, urlStr, params) {
    if (params.nick) {
      let user = await N.models.users.User.findOne()
                           .where('nick_normalized_lc').equals(params.nick.toLowerCase())
                           .select('hid')
    ...
    // falls back on entry (no comment hid)
    let fallback_link;
    ...
    let mapping = await N.models.vbconvert.BlogTextMapping.findOne()
                            .where('blogtextid').equals(params.comment)
                            .select('mongo')
                            .lean(true);

    if (!mapping) return fallback_link;

    let comment = await N.models.blogs.BlogComment.findById(mapping.mongo)
                            .select('hid entry')
                            .lean(true);

    if (!comment) return fallback_link;

    let entry = await N.models.blogs.BlogEntry.findById(comment.entry)
                          .select('hid user')
                          .lean(true);
  ...
  //
  async function link_to_blog_category(N, urlStr, params) {
    if (params.nick) {
      let user = await N.models.users.User.findOne()
                           .where('nick_normalized_lc').equals(params.nick.toLowerCase())
                           .select('hid')
    ...
    // falls back to user blog list
    let fallback_link = null;
    ...
    let mapping = await N.models.vbconvert.BlogCategoryMapping.findOne()
                            .where('mysql').equals(params.category)
                            .select('mongo')
                            .lean(true);

    if (!mapping) return fallback_link;

    let tag = await N.models.blogs.BlogTag.findById(mapping.mongo)
                        .select('hid user')
                        .lean(true);

    if (!tag) return fallback_link;

    let user = await N.models.users.User.findById(tag.user)
                         .select('hid')
                         .lean(true);
  ...
  // Links to clubs
  //
  function link_to_group_index(/*N, urlStr, params*/) {
    return { apiPath: 'clubs.index' };
  }


  // Links to single club
  //
  function link_to_group(N, urlStr, params) {
    return {
      apiPath: 'clubs.sole',
      params:  { club_hid: params.group }
    };
  }
  ...
  // Links to club topic
  //
  async function link_to_group_thread(N, urlStr, params) {
    let post_hid;

    // user can specify posts per page with ?pp=XX, but we ignore it
    // 'cause it's always 40 anyway in real links
    if (params.page) {
      post_hid = (params.page - 1) * POSTS_PER_PAGE + 1;
    }
    ...
    let fallback_link = {
      apiPath: 'clubs.topic',
      params: {
        club_hid:  params.group || PH_CLUB_HID,
        topic_hid: params.thread,
        post_hid
      }
    };
    ...
    let club = await N.models.clubs.Club.findById(topic.club)
                         .select('hid')
                         .lean(true);

    if (!club) return fallback_link;
  ...
  // Links to club post
  //
  async function link_to_group_post(ldb, urlStr, params) {
    let post_mapping = await N.models.vbconvert.ClubPostMapping.findOne()
                                 .where('mysql').equals(params.post)
                                 .lean(true);

    if (!post_mapping) return link_to_group_thread(N, urlStr, params);

    let topic = await N.models.clubs.Topic.findById(post_mapping.topic_id)
                          .select('hid club')
                          .lean(true);

    if (!topic) return link_to_group_thread(N, urlStr, params);

    let club = await N.models.clubs.Club.findById(topic.club)
                         .select('hid')
                         .lean(true);

    if (!club) return link_to_group_thread(N, urlStr, params);
  ...
  let rules = {
    ...
    // blog comments
    '/blogs/:user/blog:entry-page:page.html#comment:comment': link_to_blog_comment,
    '/blogs/:user/blog:entry.html#comment:comment':    link_to_blog_comment,
    '/blogs/comments/comment:comment.html':            link_to_blog_comment,
    '/entry.php?b=:entry&bt=:comment':                 link_to_blog_comment,
    '/entry.php?bt=:comment':                          link_to_blog_comment,

    // blog entries
    '/blogs/:user/blog:entry-page:page.html':          link_to_blog_entry,
    '/blogs/:user/blog:entry.html':                    link_to_blog_entry,
    '/entry.php?b=:entry':                             link_to_blog_entry,

    // blog categories
    '/blogs/:user/category:category/index:page.html':  link_to_blog_category,
    '/blogs/:user/category:category':                  link_to_blog_category,

    // blog users
    '/blogs/:user/index:page.html':                    link_to_blog_user,
    '/blogs/:user/custom:id.html':                     link_to_blog_user,
    '/blogs/:user':                                    link_to_blog_user,

    // redirects to blogs root
    '/blogs/recent-entries/index:page.html':           link_to_blog_index,
    '/blogs':                                          link_to_blog_index,

    // some kind of legacy version with nickname instead of user
    '/blog/:nick([^/]+)/index.php?showentry=:entry#comment:comment': link_to_blog_comment,
    '/blog/:nick([^/]+)/index.php?showentry=:entry':      link_to_blog_entry,
    '/blog/:nick([^/]+)/index.php?eid=:entry':            link_to_blog_entry,
    '/blog/:nick([^/]+)/index.php?cat=:category':         link_to_blog_category,
    '/blog/:nick([^/]+)/index.php':                       link_to_blog_user,
    '/blog/:nick([^/]+)':                                 link_to_blog_user,

    // clubs
    '/groups':                                            link_to_group_index,
    '/groups/:group':                                     link_to_group,
    '/groups/:group/:thread/index:page.html#gmessage:post': link_to_group_post,
    '/groups/:group/:thread#gmessage:post':               link_to_group_post,
    '/groups/:group/:thread/index:page.html':             link_to_group_thread,
    '/groups/:group/:thread':                             link_to_group_thread,
    '/groups/:group/pictures/:attachmentid':              link_to_attach,
  ...
  //
  // NOTE: this is a hack that doesn't work in path-to-regexp@3,
  //       need to figure out a different way to default to \d+
  //
