...
module.exports = function (N) {
  ...
  return async function parse_bbcode(posts) {
    ...
    posts.forEach(post => {
      ...
      //
      // Append vb attachments to text if they aren't already there
      //
      let inline_attachments = tokens.filter(t => t.type === 'attach').map(t => Number(t.param));
      let tail_attachments = _.difference(post.attachments, inline_attachments);

      if (tail_attachments.length > 0) {
        tokens.push({ type: 'newline', text: '\n' });
        tokens.push({ type: 'newline', text: '\n' });

        for (let attach of tail_attachments) {
          tokens.push({
            type: 'attach',
            param: String(attach),
            text: `[ATTACH]${attach}[/ATTACH]`
          });
        }
      }
