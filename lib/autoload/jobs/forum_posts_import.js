  ...
  N.wire.on('init:jobs', function register_forum_posts_import() {
    N.queue.registerWorker({
      ...
      * process() {
        ...
        let link_params = results[3];
        let link_map    = {};

        // Build internal links (link map)
        //
        Object.keys(link_params).forEach(link => {
          let t = link_params[link], result;

          if (_.isObject(t)) {
            result = N.router.linkTo(t.apiPath, t.params, linkDefaults);

            if (t.hash) result += t.hash;
          } else {
            result = t;
          }

          link_map[link] = result;
        });
        ...
        Promise.map(posts, co.wrap(function* (post_ref) {
          ...
          post_ref.tokens.forEach(function (token) {
            ...
            let link = link_params[token.param];
